...
const app = createApp({
  setup() {
    ...
    const openEditFAModal = (fa) => {
      ...
      editFAForm.date =
        fa.purchase_date ||
        (typeof getLocalISODate === "function" ? getLocalISODate() : "");
      editFAForm.cost = fa.original_cost;
      editFAForm.months = fa.monthly_depreciation
        ? Math.round(fa.original_cost / fa.monthly_depreciation)
        : 60;
    ...
    const saveEditFA = () => {
      ...
      if (initTx) {
        ...
        if (initTx.debits && initTx.debits[0])
          initTx.debits[0].amount = editFAForm.cost;
        if (initTx.credits && initTx.credits[0])
          initTx.credits[0].amount = editFAForm.cost;
    ...
    const executeDeleteFAFromModal = () => {
      if (
        !confirm(
          "確定要刪除此固定資產嗎？這將會同步刪除「當時期初建檔」與「所有自動折舊」的交易紀錄！",
        )
      )
        return;
      ...
      data.transactions = data.transactions.filter(
        (t) => !(t && t.asset_id === editFAForm.id),
      );

      // 2. 找出並刪除期初建檔明細 (這會連帶觸發 deleteTransaction 內部邏輯刪除 fixed_assets 陣列)
      let initTx = data.transactions.find(
        (t) => t && t.fa_init_id === editFAForm.id,
      );
      if (initTx) {
    ...
    const qaList = ref([
      ...
      {
        q: "Q8：如何使用「多帳本」功能？",
        a: "多帳本可以讓您將「公司帳」與「個人帳」完全獨立。在「設定 > 帳本管理與切換」中可建立新帳本，每個帳本皆擁有獨立的資產、明細與 Google Drive 備份檔，切換時系統會自動加密存檔當前進度。",
      },
      {
        q: "Q9：什麼是「高頻記帳範本」？",
        a: "在「設定 > 記帳範本」中，您可以預先將常買的咖啡、便當或固定的轉帳設定好名稱與金額。設定完成後，在「記帳」頁面頂部會出現專屬的閃電按鈕，點擊即可一鍵帶入所有資料！",
      },
      {
        q: "Q10：每個月固定的房租或訂閱費可以自動記嗎？",
        a: "當然可以！請前往左側選單的「週期排程」，建立您的每月固定收支項目並指定扣款日與帳戶。系統會在每天啟動時自動檢查，若時間到達便會自動幫您入帳。",
      },
      {
        q: "Q11：車貸或房貸該如何記帳？",
        a: "建議先在「資產與貸款」中新增「貸款項目」，輸入您的本金與利率。接著在記帳頁面選擇「貸款」模式進行每月還款，系統內建的「智慧本息自動拆分預覽」會自動幫您算出這筆錢有多少是用來還本金、多少是被扣除的利息費用！",
      },
      {
        q: "Q12：買了手機或電腦，如何使用「折舊」功能？",
        a: "當您在記帳時勾選「登錄為動產/固定資產」，這筆花費就不會一次性重擊您當月的損益表！它會被列入固定資產中，系統會依據您設定的「折舊月數」，每個月自動攤提費用，讓財務報表更平穩。",
      },
      {
        q: "Q13：股票的配股配息該怎麼記？",
        a: "在記帳頁面選擇「投資」模式，點擊「領配息」或「領配股」。若是配息，資金會真實流入您的銀行帳戶；若是配股，系統則會啟動備忘錄機制，僅增加您的庫存股數並自動攤平每股均價，不會影響現金流。",
      },
      {
        q: "Q14：預算管理是如何計算的？",
        a: "您必須先在設定中建立「支出主類別」（如：飲食、交通），然後在「預算管理」頁面為這些主類別設定每月上限。系統會依據您設定的「結算起算日」自動抓取當期區間的所有相關消費，並提供綠色/紅色的進度條視覺警告。",
      },
      {
        q: "Q15：「專案與分帳中心」怎麼用？",
        a: "當您與朋友出遊或合資時，可以建立一個「群組結算專案」。在裡面您可以隨意記錄「誰先代墊了什麼」，系統會自動利用演算法算出「最佳轉帳指示」（誰該給誰多少錢）。結算完畢後，還能一鍵將您應負擔的費用精準寫入您自己的帳本中！",
      },
    ]);
    ...
    const showDailyReminder = computed(() => {
      ...
      let hasTodayTx = (data.transactions || []).some(
        (tx) => tx && tx.date === today && !tx.auto_generated,
      );
    ...
    const projectBudgetForm = reactive({
      name: "",
      tag: "",
      limit: null,
      startDate: "",
    ...
    const groupSplitProjectForm = reactive({
      id: "",
      name: "",
      members: [{ name: "我" }, { name: "" }],
    });
    const groupSplitRecordForm = reactive({
      id: "",
      desc: "",
      expenseAcc: "",
      advanceAcc: "",
    ...
    const activeSplitProject = computed(() =>
      (data.split_projects || []).find(
        (p) => p.id === activeSplitProjectId.value,
      ),
    );
    const activeSplitRecords = computed(() =>
      (data.split_records || []).filter(
        (r) => r.project_id === activeSplitProjectId.value,
      ),
    );

    const activeSplitBalances = computed(() => {
      if (!activeSplitProject.value) return {};
      return typeof calculateNetBalances === "function"
        ? calculateNetBalances(
            activeSplitRecords.value,
            activeSplitProject.value.members,
          )
        : {};
    });

    const activeSplitSettlements = computed(() => {
      return typeof optimizeSettlements === "function"
        ? optimizeSettlements(activeSplitBalances.value)
        : [];
    });

    const openGroupSplitCenter = () => {
      activeTab.value = "group_split";
      isDrawerOpen.value = false;
    };
    const viewGroupSplitProject = (id) => {
      activeSplitProjectId.value = id;
    };
    const backToSplitProjects = () => {
      activeSplitProjectId.value = "";
    };

    const addSplitMemberField = () => {
      groupSplitProjectForm.members.push({ name: "" });
    };
    const removeSplitMemberField = (idx) => {
      groupSplitProjectForm.members.splice(idx, 1);
    };
    ...
    const saveGroupSplitProject = () => {
      ...
      let validMembers = groupSplitProjectForm.members.filter(
        (m) => (m.name || "").trim() !== "",
      );
      if (!validMembers.find((m) => m.name === "我"))
        validMembers.unshift({ name: "我" });

      if (groupSplitProjectForm.id) {
        // 編輯模式
        let p = data.split_projects.find(
          (x) => x && x.id === groupSplitProjectForm.id,
        );
        if (p) {
          p.name = groupSplitProjectForm.name;
      ...
      groupSplitProjectForm.name = "";
      groupSplitProjectForm.members = [{ name: "我" }, { name: "" }];
    ...
    const deleteGroupSplitProject = (id) => {
      ...
      data.split_records = data.split_records.filter(
        (r) => r.project_id !== id,
      );
    ...
    const initGroupSplitRecordForm = () => {
      ...
      groupSplitRecordForm.desc = "";
      groupSplitRecordForm.expenseAcc = "";
      groupSplitRecordForm.advanceAcc = "";
      groupSplitRecordForm.payer = "我";
      groupSplitRecordForm.amount = null;
      groupSplitRecordForm.mode = "even";
      groupSplitRecordForm.splits = activeSplitProject.value.members.map(
        (m) => ({ member: m.name, amount: null, included: true }),
      );
    ...
    const calculateGroupSplitRecord = () => {
      if (groupSplitRecordForm.mode === "even") {
        let includedCount = groupSplitRecordForm.splits.filter(
          (s) => s.included,
        ).length;
    ...
    watch(() => groupSplitRecordForm.splits, calculateGroupSplitRecord, {
      deep: true,
    });
    watch(
      () => newTx.subAccount,
    ...
    const saveGroupSplitRecord = () => {
      if (
        !groupSplitRecordForm.desc ||
        !groupSplitRecordForm.amount ||
        !groupSplitRecordForm.expenseAcc
      )
        return alert("請填寫完整項目、金額與支出科目");
      if (
        groupSplitRecordForm.payer === "我" &&
        !groupSplitRecordForm.advanceAcc
      )
        return alert("請選擇您先代墊付款的實體帳戶/信用卡");

      let totalSplit = groupSplitRecordForm.splits.reduce(
        (sum, s) => sum + (Number(s.amount) || 0),
        0,
      );
      if (Math.abs(totalSplit - groupSplitRecordForm.amount) > 0)
        return alert("成員分攤總額與該筆總金額不符，請檢查分配金額。");

      if (groupSplitRecordForm.id) {
        // 編輯模式
        let rec = data.split_records.find(
          (r) => r.id === groupSplitRecordForm.id,
        );
        if (rec) {
        ...
        data.split_records.push({
          id: "gsr_" + Date.now(),
          project_id: activeSplitProjectId.value,
          date: getLocalISODate(),
          desc: groupSplitRecordForm.desc,
          expenseAcc: groupSplitRecordForm.expenseAcc,
          advanceAcc: groupSplitRecordForm.advanceAcc,
          payer: groupSplitRecordForm.payer,
          amount: groupSplitRecordForm.amount,
          splits: JSON.parse(JSON.stringify(groupSplitRecordForm.splits)),
        });
      }
      showGroupSplitRecordModal.value = false;
      autoBackup(true, true);
    };

    const deleteGroupSplitRecord = (id) => {
      if (confirm("確定刪除此筆代墊紀錄？")) {
        data.split_records = data.split_records.filter((r) => r.id !== id);
        autoBackup(true, true);
      }
    };
    ...
    const generateMagicLink = () => {
      ...
      let payload = {
        ...
        r: activeSplitRecords.value.map((rec) => ({
          d: rec.desc,
          p: rec.payer,
          a: rec.amount,
        })),
        s: activeSplitSettlements.value.map((stl) => ({
          f: stl.from,
          t: stl.to,
          a: stl.amount,
        })),
      ...
      let b64 =
        typeof b64EncodeUnicode === "function"
          ? b64EncodeUnicode(JSON.stringify(payload))
          : "";
      return b64
    ...
    const shareGroupSettlement = async () => {
      ...
      activeSplitRecords.value.forEach((r) => {
        text += `- ${r.desc}: ${r.payer} 先付 $${formatNumber(r.amount)}\n`;
      });
      text += `\n📊 最佳轉帳路徑：\n`;
      if (activeSplitSettlements.value.length === 0) {
        text += `✅ 大家互不相欠！\n`;
      } else {
        activeSplitSettlements.value.forEach((s) => {
          text += `👉 [${s.from}] 需轉帳給 [${s.to}] $${formatNumber(s.amount)}\n`;
        });
      }
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            title: activeSplitProject.value.name,
            text: text,
          });
        } catch (e) {}
      } else {
        ...
        try {
          ...
          alert(
            "✅ 結算報告已複製至剪貼簿！可直接貼上至 LINE 或其他通訊軟體。",
          );
        } catch (err) {
    ...
    const checkSharedUrl = () => {
      try {
        ...
        if (sharedObj) {
          let decoded =
            typeof b64DecodeUnicode === "function"
              ? b64DecodeUnicode(sharedObj)
              : "{}";
    ...
    const writeGroupSettlementToLedger = () => {
      if (!activeSplitProject.value) return;
      if (!groupSettleLedgerForm.settleAcc)
        return alert("請選擇結算轉帳收付款用的實體帳戶");
      ...
      activeSplitRecords.value.forEach((r) => {
        if (r.payer === "我") {
          ...
          let advAcc =
            r.advanceAcc || groupSettleLedgerForm.advanceAcc || "1101"; // 防呆預設為現金
          advanceCreditsMap[advAcc] =
            (advanceCreditsMap[advAcc] || 0) + Number(r.amount);
        }
        ...
        if (mySplit && Number(mySplit.amount) > 0) {
          ...
          myExpensesByCategory[expAcc] =
            (myExpensesByCategory[expAcc] || 0) + amt;
        }
      });

      if (myTotalPaid === 0 && myTotalExpense === 0)
        return alert("您在此專案中沒有任何花費與代墊，無需寫入帳本。");

      let txObj = {
        id: "tx_gsp_" + Date.now(),
        date: getLocalISODate(),
        scope: "personal",
        desc: `[群組結算] ${activeSplitProject.value.name}`,
        tags: [activeSplitProject.value.name.replace(/\s+/g, "")],
        debits: [],
        credits: [],
      };
      ...
      for (let accId in myExpensesByCategory) {
        txObj.debits.push({
          account_id: accId,
          amount: myExpensesByCategory[accId],
        });
      }
      ...
      if (myBalance > 0) {
        ...
        txObj.credits.push({
          account_id: groupSettleLedgerForm.settleAcc,
      ...
      const consolidate = (entries) => {
        ...
        return Object.keys(map)
          .map((k) => ({ account_id: k, amount: map[k] }))
          .filter((e) => e.amount > 0);
      };
    ...
    const calcConfirm = () => {
      let res =
        typeof evaluateCalc === "function"
          ? evaluateCalc(calcExpression.value)
          : null;
    ...
    const startVoiceRecognition = () => {
      ...
      try {
        ...
        recognition.onresult = (event) => {
          ...
          let parsed =
            typeof parseVoiceCommand === "function"
              ? parseVoiceCommand(transcript, data.accounts)
              : { amount: null, desc: transcript, tags: [], paymentAcc: "" };
    ...
    const onSymbolInput = (target) => {
      ...
      if (typeof STOCK_DICTIONARY !== "undefined" && STOCK_DICTIONARY[symbol]) {
        ...
        let existing = (data.investments || []).find(
          (i) =>
            i &&
            i.symbol &&
            i.symbol.replace(".TW", "").toUpperCase() === symbol,
        );
        if (existing)
    ...
    const onInvestSelectedSymbolChange = () => {
      if (
        newTx.investSelectedSymbol === "manual" ||
        !newTx.investSelectedSymbol
      ) {
        newTx.symbol = "";
        newTx.stockName = "";
      } else {
        let inv = (data.investments || []).find(
          (i) => i && i.symbol === newTx.investSelectedSymbol,
        );
        if (inv) {
    ...
    const onDividendSymbolChange = () => {
      if (
        ...
        let inv = (data.investments || []).find(
          (i) => i && i.symbol === newTx.investDividendSymbol,
        );
    ...
    const executeDeleteAccountFromModal = () => {
      ...
      data.transactions.forEach((tx) => {
        if (tx.debits)
          tx.debits.forEach((d) => {
            if (d.account_id === id) isUsed = true;
          });
        if (tx.credits)
          tx.credits.forEach((c) => {
            if (c.account_id === id) isUsed = true;
          });
      });
      if (isUsed) {
        alert(
    ...
    const addCustomCurrency = () => {
      ...
      if (!code || code.length !== 3)
        return alert("請輸入3碼英文幣別 (如: EUR, GBP)");
    ...
    const deleteCustomCurrency = (code) => {
      if (code === "USD" || code === "JPY" || code === "TWD")
        return alert("系統預設幣別無法刪除");
      if (
        !confirm(
          `確定要刪除幣別 ${code} 嗎？\n(注意：若有使用該幣別的帳戶，換算可能會受影響)`,
        )
      )
        return;
        ...
        if (apiData && apiData.rates) {
          for (let cur in data.currencyRates) {
            if (cur !== "TWD" && apiData.rates[cur]) {
              ...
              data.currencyRates[cur] = Number(
                (1 / apiData.rates[cur]).toFixed(4),
              );
            }
          }
          autoBackup();
    ...
    const activeBookName = computed(() => {
      let b = settings.booksIndex.find(
        (x) => x && x.id === currentBookId.value,
      );
      return b ? b.name : "Kadu｜卡度記帳";
    });
    ...
    const activeInstallments = computed(() =>
      (data.installments || []).filter((i) => i && i.paid_periods < i.periods),
    );
    const getSubAccounts = (type, mainCat, incHidden = false) =>
      (data.accounts || []).filter(
        (a) =>
          a &&
          a.type === type &&
          (!mainCat || a.category === mainCat) &&
          (incHidden || !a.is_hidden),
      );

    const safeQuickTags = computed(() => data.quick_tags || []);
    const activeProjectTags = computed(() => {
      let tags = [];
      (data.project_budgets || []).forEach((p) => {
        if (p && p.tag) tags.push(p.tag);
      });
      (data.split_projects || []).forEach((p) => {
        if (p && !p.is_settled && p.name) tags.push(p.name.replace(/\s+/g, ""));
      });
      (data.savings_goals || []).forEach((g) => {
        if (g && g.tag) tags.push(g.tag);
        else if (g && g.name) tags.push(g.name.replace(/\s+/g, ""));
      });
      return [...new Set(tags)];
    });
    ...
    const combinedQuickTags = computed(() => {
      return {
        ...
        presets: (data.quick_tags || []).filter(
          (t) => !activeProjectTags.value.includes(t),
        ),
      };
    });
        ...
        .filter((tx) => {
          ...
          if (tx.debits && tx.debits.length > 0) {
            let acc = (data.accounts || []).find(
              (a) => a && a.id === tx.debits[0].account_id,
            );
    ...
    const applyRecentTx = (tx) => {
      ...
      if (tx.debits && tx.debits[0]) {
        let expAcc = (data.accounts || []).find(
          (a) => a && a.id === tx.debits[0].account_id,
        );
        if (expAcc) {
          newTx.mainCategory = expAcc.category || "";
          newTx.subAccount = expAcc.id || "";
        }
      }
      if (tx.credits && tx.credits[0])
        newTx.paymentAcc = tx.credits[0].account_id || "";
    };
    const safeInvestments = computed(() => data.investments || []);
    const safeFixedAssets = computed(() =>
      (data.fixed_assets || []).filter((fa) => fa && !fa.is_disposed),
    );
    ...
    const historicalHoldings = computed(() => {
      ...
      safeInvestments.value.forEach((inv) => {
        if (inv && inv.shares === 0 && !currentSyms.includes(inv.symbol))
          hist.push({
            symbol: inv.symbol,
            name: `[${inv.symbol}] ${(inv.name || "").replace(/^\[.*?\]\s*/, "")}`,
          });
      });
      return hist;
    });
    ...
    const calculateBalance = (id, scope = "all") => {
      ...
      (data.transactions || []).forEach((tx) => {
        ...
        if (tx.debits)
          tx.debits.forEach((d) => {
            if (d && d.account_id === id) bal += Number(d.amount) || 0;
          });
        if (tx.credits)
          tx.credits.forEach((c) => {
            if (c && c.account_id === id) bal -= Number(c.amount) || 0;
          });
      });
      let acc = (data.accounts || []).find((a) => a && a.id === id);
      return acc && (acc.type === "Asset" || acc.type === "Expense")
        ? bal
        : -bal;
    };
    ...
    const paymentAccountsWithBalance = computed(() =>
      accountsWithBalance(paymentAccounts.value),
    );
    const assetAccountsWithBalance = computed(() =>
      accountsWithBalance(assetAccounts.value),
    );
    const liquidAccountsWithBalance = computed(() =>
      accountsWithBalance(assetAccounts.value),
    );
    const liabilityAccountsWithBalance = computed(() =>
      accountsWithBalance(liabilityAccounts.value),
    );

    const totalLiquidAssets = computed(() =>
      liquidAccountsWithBalance.value.reduce(
        (s, acc) => s + (acc.baseBalance || 0),
        0,
      ),
    );
    ...
    const upcomingBillsTotal = computed(() => {
      ...
      liabilityAccountsWithBalance.value.forEach((acc) => {
        if (acc.baseBalance < 0) sum += Math.abs(acc.baseBalance);
      });
      activeInstallments.value.forEach((inst) => {
        if (inst && inst.next_month <= cM)
          sum += Number(inst.amount_per_period) || 0;
      });
      safeLoans.value.forEach((loan) => {
        if (loan) sum += Number(loan.monthly_payment) || 0;
      });
      safeRecurring.value.forEach((rec) => {
        if (rec && rec.type === "expense") sum += Number(rec.amount) || 0;
      });
      return sum;
    });

    const cashflowWarning = computed(
      () => totalLiquidAssets.value < upcomingBillsTotal.value * 1.2,
    );
    ...
    const totalAssets = computed(() => {
      ...
      (data.accounts || []).forEach((a) => {
        if (a && a.type === "Asset")
          sum += getBaseBalance(a.id, calculateBalance(a.id, scope));
      });
    ...
    const activeBillingPeriod = computed(() => {
      return typeof getCurrentBillingPeriod === "function"
        ? getCurrentBillingPeriod(
            dashboardMonth.value + "-01",
            settings.billingStartDay || 1,
          )
        : {
            startDate: dashboardMonth.value + "-01",
            endDate: dashboardMonth.value + "-31",
          };
    });
      ...
      (data.transactions || []).forEach((tx) => {
        if (
          ...
          if (
            ...
            return;
          (tx.debits || []).forEach((d) => {
            let a = (data.accounts || []).find(
              (ac) => ac && ac.id === d.account_id,
            );
            if (a && a.type === "Expense") {
              let cat = a.category || "未分類";
              if (!expObj[cat]) expObj[cat] = 0;
              expObj[cat] += Number(d.amount) || 0;
            }
          });
        }
      });
      for (let cat in data.budgets || {}) {
        let limit = Number(data.budgets[cat]) || 0;
        if (limit <= 0) continue;
    ...
    const budgetStats = computed(() => {
      let limit = 0,
        spent = 0;
      for (let cat in dashboardBudgets.value) {
        limit += dashboardBudgets.value[cat].limit;
        spent += dashboardBudgets.value[cat].spent;
      }
      ...
      return {
        ...
        dailyRemaining: remaining > 0 ? Math.floor(remaining / daysLeft) : 0,
        daysLeft,
      };
    });
    ...
    const filteredTransactions = computed(() => {
      return sortedTransactions.value.filter((tx) => {
        if (!tx) return false;
        let kw = historyFilter.keyword.toLowerCase(),
          desc = getTxDesc(tx).toLowerCase(),
          accD = getDebitAccName(tx).toLowerCase(),
          accC = getCreditAccName(tx).toLowerCase();

        // 修正：動態在比對時為 tx.tags 陣列元素補上 '#' 符號，確保專案跳轉過濾能 100% 命中
        let matchTags = (tx.tags || [])
          .map((t) => "#" + t)
          .join(" ")
          .toLowerCase()
          .includes(kw);
        ...
        let matchKw =
          ...
          matchTags;
        let matchScope =
          historyFilter.scope === "all" || tx.scope === historyFilter.scope;
        let matchDate =
          (!historyFilter.dateFrom || tx.date >= historyFilter.dateFrom) &&
          (!historyFilter.dateTo || tx.date <= historyFilter.dateTo);
    ...
    const ytdDividend = computed(() => {
      ...
      (data.transactions || []).forEach((tx) => {
        if (
          tx &&
          tx.date &&
          tx.date.startsWith(y) &&
          tx.credits &&
          !tx.is_refunded &&
          !tx.is_refund
        ) {
          tx.credits.forEach((c) => {
            if (c && c.account_id === "4202") sum += Number(c.amount) || 0;
          });
        }
      });
      return sum;
    ...
    const expenseCategories = computed(() =>
      data.main_categories && data.main_categories.Expense
        ? data.main_categories.Expense
        : [],
    );
    const incomeCategories = computed(() =>
      data.main_categories && data.main_categories.Income
    ...
    const getAccName = (id) => {
      ...
      return a ? (a.icon ? `${a.icon} ${a.name}` : a.name) : id || "未知";
    };
    const getTxDesc = (tx) =>
      tx && (tx.desc || tx.description) ? tx.desc || tx.description : "無摘要";
    ...
    const getDebitAccType = (tx) => {
      if (tx && tx.debits && tx.debits[0]) {
        let a = data.accounts.find(
          (ac) => ac && ac.id === tx.debits[0].account_id,
        );
        return a ? a.type : "";
      }
      return "";
    ...
    const getInvestTotalAmount = () => {
      ...
      return newTx.investAction === "buy"
        ? base + (Number(newTx.fee) || 0)
        : base - (Number(newTx.fee) || 0) - (Number(newTx.tax) || 0);
    };
    const getInvCurrentValue = (inv) =>
      inv
        ? (Number(inv.shares) || 0) *
          (Number(inv.last_price) || 0) *
          (data.currencyRates[inv.currency || "TWD"] || 1)
        : 0;
    const getUnrealizedGain = (inv) =>
      inv ? getInvCurrentValue(inv) - (Number(inv.total_cost) || 0) : 0;
    const getFAAccDep = (fa) => {
      ...
      (data.transactions || []).forEach((tx) => {
        if (tx && tx.asset_id === fa.id && !tx.is_refunded && !tx.is_refund) {
          (tx.credits || []).forEach((c) => {
            if (c && c.account_id === fa.accumulated_dep_account_id)
    ...
    const getFABookValue = (fa) =>
      fa ? (Number(fa.original_cost) || 0) - getFAAccDep(fa) : 0;
    const getAccumulatedInterest = (loanId) => {
      ...
      (data.transactions || []).forEach((tx) => {
        if (
          ...
          tx.debits.forEach((d) => {
            if (d && d.account_id === "5103") sum += Number(d.amount) || 0;
          });
        }
      });
      return sum;
    ...
    const loanRepayPreview = computed(() => {
      ...
      if (!loan || !baseAmt)
        return { interest: 0, principal: 0, current_principal: 0 };
    ...
    const getTxColorBand = (tx) => {
      ...
      if (tx && tx.debits && tx.debits[0]) {
        let acc = list.find((a) => a && a.id === tx.debits[0].account_id);
        if (acc) tD = acc.type;
      }
      if (tx && tx.credits && tx.credits[0]) {
        let acc = list.find((a) => a && a.id === tx.credits[0].account_id);
        if (acc) tC = acc.type;
      }
    ...
    const getTxAmountColor = (tx) => {
      ...
      if (tD === "Asset" && !getTxDesc(tx).includes("轉帳"))
        return "text-green-600 dark:text-green-400";
      return "text-slate-700 dark:text-slate-300";
    };

    const applyQuickTag = (tag) => {
      if (newTx.desc && !newTx.desc.includes(`#${tag}`))
        newTx.desc += ` #${tag}`;
    ...
    watch(
      ...
      (newVal) => {
        ...
        if (newVal === "this_month") {
          reportStartDate.value = getLocalISODate(
            new Date(d.getFullYear(), d.getMonth(), 1),
          );
          reportEndDate.value = getLocalISODate(
            new Date(d.getFullYear(), d.getMonth() + 1, 0),
          );
        } else if (newVal === "this_quarter") {
          let q = Math.floor(d.getMonth() / 3);
          reportStartDate.value = getLocalISODate(
            new Date(d.getFullYear(), q * 3, 1),
          );
          reportEndDate.value = getLocalISODate(
            new Date(d.getFullYear(), q * 3 + 3, 0),
          );
    ...
    const submitTransaction = () => {
      ...
      extractedTags.forEach((tag) => {
        let matchedGoal = (data.savings_goals || []).find(
          (g) => g && (g.name === tag || g.tag === tag),
        );
      ...
      if (entryMode.value === "expense") {
        if (!newTx.paymentAcc || !baseAmt)
          return (txError.value = "請填寫完整金額與扣款帳戶");
        let debitAcc = newTx.isReimbursement ? "1104" : newTx.subAccount;
        if (!debitAcc) return (txError.value = "請選擇分類或勾選代墊");
        if (!newTx.isReimbursement && newTx.desc) {
          if (!data.smart_tags) data.smart_tags = {};
          data.smart_tags[newTx.desc] = newTx.paymentAcc;
        }
        ...
        if (newTx.isInst && newTx.periods > 1) {
          ...
          let nextM =
            newTx.date && newTx.date.length >= 7
              ? newTx.date.substring(0, 7)
              : getLocalISODate().substring(0, 7);
          let nextD =
            newTx.date && newTx.date.length >= 10
              ? newTx.date.substring(8, 10)
              : "01";
          data.installments.push({
            id: "inst_" + Date.now(),
            desc: newTx.desc || "無摘要",
            total_amount: baseAmt,
            periods: newTx.periods,
            amount_per_period: perAmt,
            first_amount: firstAmt,
            paid_periods: 0,
            next_month: nextM,
            date_day: nextD,
            debit_acc: debitAcc,
            credit_acc: newTx.paymentAcc,
            scope: newTx.scope,
          });
          runAutoTasks();
          newTx.amount = null;
        ...
        if (!newTx.isReimbursement && newTx.isFA && newTx.faMonths > 0) {
          ...
          data.fixed_assets.push({
            id: newFaId,
            name: newTx.faName || newTx.desc,
            purchase_date: newTx.date,
            original_cost: baseAmt,
            monthly_depreciation: monthlyDep,
            asset_account_id: "1201",
            accumulated_dep_account_id: "1201-DEP",
            expense_account_id: "5102",
            last_depreciation_date: newTx.date,
            is_disposed: false,
          });
        ...
        if (!newTx.subAccount || !newTx.paymentAcc || !baseAmt)
          return (txError.value = "欄位不完整");
        if (newTx.desc) {
          if (!data.smart_tags) data.smart_tags = {};
          data.smart_tags[newTx.desc] = newTx.paymentAcc;
        }
        txObj.debits.push({ account_id: newTx.paymentAcc, amount: baseAmt });
        txObj.credits.push({ account_id: newTx.subAccount, amount: baseAmt });
      } else if (entryMode.value === "transfer") {
        if (!newTx.fromAcc || !newTx.toAcc || !baseAmt)
          return (txError.value = "欄位不完整");
        if (newTx.fromAcc === newTx.toAcc)
          return (txError.value = "轉出入不可相同");
        ...
        if (newTx.investAction === "dividend") {
          ...
          if (inv) {
            ...
            data.investments.push({
              id: "inv_" + Date.now(),
              symbol: newTx.symbol,
              name: newTx.stockName || newTx.symbol,
              shares: newTx.shares,
              total_cost: 0,
              last_price: 0,
              currency: "TWD",
          ...
          if (newTx.investAction === "buy") {
            ...
            if (!inv || inv.shares < newTx.shares)
              return (txError.value = "賣出股數不可超過庫存");
            let costProp =
              inv.shares > 0
                ? Math.round(inv.total_cost * (newTx.shares / inv.shares))
                : 0;
            ...
            if (gain > 0) {
              txObj.credits.push({ account_id: "4201", amount: gain });
            } else if (gain < 0) {
              txObj.debits.push({ account_id: "4201", amount: Math.abs(gain) });
            }
            inv.shares -= newTx.shares;
            inv.total_cost -= costProp;
          }
        }
      } else if (entryMode.value === "loan_repay") {
        if (!newTx.loanId || !newTx.paymentAcc || !baseAmt)
          return (txError.value = "欄位不完整");
        let loan = (data.loans || []).find((l) => l && l.id === newTx.loanId);
        if (!loan) return (txError.value = "貸款資料錯誤");
        let preview = loanRepayPreview.value;
        txObj.loan_id = loan.id;
        txObj.desc = newTx.desc || `貸款還款: ${loan.name}`;
        if (newTx.currency !== "TWD")
          txObj.desc += ` (${newTx.currency} ${newTx.amount})`;
        txObj.debits.push({
          account_id: loan.liability_acc_id,
    ...
    const executeRollover = () => {
      ...
      const oldTxs = data.transactions.filter(
      ...
      oldTxs.forEach((tx) => {
        (tx.debits || []).forEach((d) => {
          rawBalances[d.account_id] =
            (rawBalances[d.account_id] || 0) + Number(d.amount);
        });
        (tx.credits || []).forEach((c) => {
          rawBalances[c.account_id] =
            (rawBalances[c.account_id] || 0) - Number(c.amount);
        });
      });
      ...
      (data.accounts || []).forEach((acc) => {
        if (acc && (acc.type === "Asset" || acc.type === "Liability")) {
          ...
          if (bal < 0)
            rolloverCredits.push({ account_id: acc.id, amount: Math.abs(bal) });
          targetEquity -= bal;
        }
      });

      if (targetEquity > 0)
        rolloverCredits.push({ account_id: "3101", amount: targetEquity });
      if (targetEquity < 0)
        rolloverDebits.push({
          account_id: "3101",
          amount: Math.abs(targetEquity),
        });
      ...
      updateCharts();
      alert(
        `✅ 結轉成功！已安全釋放空間，並清除 ${cutoffDate} (含) 之前的歷史明細。`,
      );
    };
    const submitNewAssetAccount = () => {
      if (!newAssetAcc.name) return;
      let finalType =
        newAssetAcc.name.includes("信用卡") ||
        newAssetAcc.name.includes("欠款") ||
        newAssetAcc.name.includes("貸款")
          ? "Liability"
          : newAssetAcc.type || "Asset";
      const newId =
        (finalType === "Liability" ? "liab_" : "asset_") + Date.now();
      let finalIcon =
        newAssetAcc.icon || (finalType === "Liability" ? "💳" : "🏦");

      data.accounts.push({
        id: newId,
        name: newAssetAcc.name,
        type: finalType,
        currency: newAssetAcc.currency,
        is_hidden: false,
        icon: finalIcon,
        billing_day: finalType === "Liability" ? newAssetAcc.billingDay : null,
      });
      ...
      if (newAssetAcc.initBalance && newAssetAcc.initBalance > 0) {
        if (finalType === "Asset") {
          data.transactions.unshift({
            ...
            date: getLocalISODate(),
            scope: "personal",
            desc: `期初餘額: ${newAssetAcc.name}`,
            debits: [{ account_id: newId, amount: newAssetAcc.initBalance }],
            credits: [{ account_id: "3101", amount: newAssetAcc.initBalance }],
          });
        } else {
          data.transactions.unshift({
            id: "tx_init_" + Date.now(),
            date: getLocalISODate(),
            scope: "personal",
            desc: `期初欠款: ${newAssetAcc.name}`,
            debits: [{ account_id: "3101", amount: newAssetAcc.initBalance }],
            credits: [{ account_id: newId, amount: newAssetAcc.initBalance }],
          });
        }
      }
      newAssetAcc.name = "";
      newAssetAcc.type = "Asset";
      newAssetAcc.initBalance = null;
      newAssetAcc.currency = "TWD";
    ...
    const submitRefund = () => {
      ...
      if (refundData.amount <= 0 || refundData.amount > refundData.maxAmount)
        return alert("輸入金額無效或大於可退餘額");
      if (!refundData.account) return alert("請選擇退款入帳帳戶");
      let expAcc =
        activeRefundTx.value.debits && activeRefundTx.value.debits[0]
          ? activeRefundTx.value.debits[0].account_id
          : null;
      ...
      activeRefundTx.value.refunded_amount =
        (Number(activeRefundTx.value.refunded_amount) || 0) + refundData.amount;
      if (
        activeRefundTx.value.refunded_amount >=
        getDebitAmount(activeRefundTx.value)
      )
    ...
    const submitReimburse = () => {
      if (!activeReimburseTx.value || !reimburseData.account)
        return alert("請選擇入帳帳戶");
      if (
        reimburseData.amount <= 0 ||
        reimburseData.amount > reimburseData.maxAmount
      )
        return alert("報銷金額無效或大於可報銷餘額");
      reimburseTx(
        activeReimburseTx.value,
        reimburseData.account,
        reimburseData.amount,
      );
      closeReimburseModal();
    };
    const reimburseTx = (tx, toAccountId, amount) => {
      ...
      data.transactions.unshift({
        ...
        debits: [{ account_id: toAccountId, amount: amount }],
        credits: [{ account_id: "1104", amount: amount }],
        ref_tx_id: tx.id,
      });
      tx.reimbursed_amount = (Number(tx.reimbursed_amount) || 0) + amount;
      if (tx.reimbursed_amount >= getDebitAmount(tx)) {
    ...
    const duplicateTransaction = (tx) => {
      ...
      newTx.currency = "TWD";
      entryMode.value =
        getDebitAccType(tx) === "Expense"
          ? "expense"
          : getDebitAccType(tx) === "Asset"
            ? "transfer"
            : "income";
      newTx.date =
        typeof getLocalISODate === "function"
      ...
      if (tx.credits && tx.credits[0]) {
        let cAcc = data.accounts.find(
          (a) => a && a.id === tx.credits[0].account_id,
        );
      ...
      if (tx.tags && tx.tags.length > 0) {
        newTx.desc +=
          (newTx.desc ? " " : "") + tx.tags.map((t) => "#" + t).join(" ");
      }
    ...
    const smartPredictEntry = () => {
      ...
      if (entryMode.value === "income") {
        const salaryAcc = data.accounts.find(
          (a) => a.name === "本薪" && a.type === "Income",
        );
        if (salaryAcc) {
          newTx.mainCategory = salaryAcc.category || "";
          newTx.subAccount = salaryAcc.id;
          newTx.desc = "本月薪資";
          const bankAcc = data.accounts.find(
            (a) =>
              a.type === "Asset" &&
              !a.is_contra &&
              a.id !== "1101" &&
      ...
      if (entryMode.value === "expense") {
        ...
        const subAcc = data.accounts.find(
          (a) => a.name === targetSubName && a.type === "Expense",
        );
        if (subAcc) {
          ...
          const recentMatch = data.transactions.find(
            (t) =>
              t.debits &&
              t.debits[0] &&
              t.debits[0].account_id === subAcc.id &&
              t.credits &&
              t.credits[0],
          );
    ...
    const startScanner = () => {
      ...
      nextTick(() => {
        ...
        const qrCodeSuccessCallback = (decodedText, decodedResult) => {
          ...
          if (
            decodedText &&
            decodedText.length >= 37 &&
            /^[A-Z]{2}\d{8}\d{7}/.test(decodedText)
          ) {
            try {
              // 1. 取得日期並轉換為西元
    ...
    const submitProjectBudget = () => {
      ...
      let tagClean = (projectBudgetForm.tag || projectBudgetForm.name)
        .replace("#", "")
        .trim();
      ...
      if (editingProjectId.value) {
        // 編輯覆蓋模式
        let p = data.project_budgets.find(
          (x) => x.id === editingProjectId.value,
        );
        if (p) {
    ...
    const deleteProjectBudget = (id) => {
      if (confirm("確定刪除此專案預算？")) {
        data.project_budgets = data.project_budgets.filter(
          (p) => p && p.id !== id,
        );
        autoBackup(true, true);
      }
      ...
      return (data.project_budgets || [])
        .map((proj) => {
          ...
          (data.transactions || []).forEach((tx) => {
            if (
              tx &&
              tx.date >= proj.startDate &&
              tx.date <= proj.endDate &&
              !tx.is_refunded &&
              !tx.is_refund
            ) {
              let hasTag =
                (tx.tags && tx.tags.includes(proj.tag)) ||
                (tx.desc && tx.desc.includes("#" + proj.tag));
              if (hasTag) {
                let txNetCost = 0;
                // 借方 (費用增加) => 專案開銷增加
                (tx.debits || []).forEach((d) => {
                  ...
                  if (a && a.type === "Expense") {
                    txNetCost += Number(d.amount) || 0;
                  }
                });
                // 貸方 (退款費用減少 或 收入增加) => 專案開銷減少 (補血)
                (tx.credits || []).forEach((c) => {
                  let a = (data.accounts || []).find(
                    (ac) => ac && ac.id === c.account_id,
                  );
                  if (a && (a.type === "Expense" || a.type === "Income")) {
                    txNetCost -= Number(c.amount) || 0;
                  }
                });
                spent += txNetCost;
              }
          ...
          return {
            ...proj,
            spent,
            remaining: proj.limit - spent,
            pct: Math.max(
              0,
              Math.min(Math.round((spent / proj.limit) * 100), 100),
            ),
          };
        })
        .filter(Boolean);
    ...
    const submitInitialStock = () => {
      ...
      if (
        !initStock.symbol ||
        s <= 0 ||
        !(initStock.cost > 0 || initStock.price > 0)
      )
      ...
      let existingInv = data.investments.find(
        (i) => i && i.symbol === initStock.symbol,
      );
      if (existingInv) {
        existingInv.shares += s;
        existingInv.total_cost += c;
        if (existingInv.shares > 0)
          existingInv.last_price = existingInv.total_cost / existingInv.shares;
      } else {
        data.investments.push({
          id: "inv_" + Date.now(),
    ...
    const submitFixedAsset = () => {
      if (!initFA.name || !initFA.cost || !initFA.months)
        return alert("請填寫完整");
      ...
      data.fixed_assets.push({
        id: newFaId,
        name: initFA.name,
        purchase_date: initFA.date,
        original_cost: initFA.cost,
        monthly_depreciation: monthlyDep,
        asset_account_id: "1201",
        accumulated_dep_account_id: "1201-DEP",
        expense_account_id: "5102",
      ...
      data.transactions.unshift({
        ...
        date: initFA.date,
        scope: initFA.scope,
        desc: `購入固定資產 ${initFA.name}`,
        debits: [{ account_id: "1201", amount: initFA.cost }],
        credits: [{ account_id: "3101", amount: initFA.cost }],
        fa_init_id: newFaId,
      });
      showAddFixedAssetModal.value = false;
      initFA.name = "";
      initFA.cost = null;
      initFA.months = 60;
      initFA.scope = "personal";
      autoBackup(true, true);
      updateCharts();
      alert("✅ 固定資產登錄成功！");
    };
    ...
    const submitDisposal = () => {
      if (
        disposalForm.type === "sell" &&
        (disposalForm.price === null || !disposalForm.account)
      )
        return alert("請填寫出售金額與入帳帳戶");
      if (
        disposalForm.type === "scrap" &&
        disposalForm.subsidyAmount > 0 &&
        !disposalForm.account
      )
        return alert("請選擇政府補助金的入帳帳戶");

      let fa = disposalAsset.value;
      if (!fa) return;
      let bookValue = getFABookValue(fa);
      ...
      let txObj = {
        ...
        scope: "family",
        desc: `處分資產: ${fa.name}`,
        debits: [{ account_id: "1201-DEP", amount: accDep }],
        credits: [{ account_id: "1201", amount: fa.original_cost }],
      };
      ...
      if (disposalForm.type === "scrap") {
        ...
        if (disposalForm.subsidyAmount > 0) {
          txObj.debits.push({
            account_id: disposalForm.account,
            amount: disposalForm.subsidyAmount,
          });
        }
    ...
    const submitAddLoan = () => {
      if (
        ...
        return alert("請填妥所有貸款欄位");
      if (
        initLoan.autoDeduct &&
        (!initLoan.deductDay || !initLoan.deductAccountId)
      )
        return alert("請填寫自動扣款日與扣繳帳戶");
      ...
      data.loans.push({
        id: loanId,
        name: initLoan.name,
        liability_acc_id: accId,
        interest_rate: initLoan.rate,
        monthly_payment: initLoan.payment,
        auto_deduct: initLoan.autoDeduct,
        deduct_day: initLoan.deductDay,
        deduct_account_id: initLoan.deductAccountId,
        last_exec_month: "",
      });

      data.transactions.unshift({
        id: "tx_loan_init_" + Date.now(),
        date: getLocalISODate(),
        scope: "personal",
        desc: `期初貸款本金: ${initLoan.name}`,
        debits: [{ account_id: "3101", amount: initLoan.principal }],
        credits: [{ account_id: accId, amount: initLoan.principal }],
        loan_init_id: loanId,
        loan_account_id: accId,
      });
      newTx.loanId = loanId;
      initLoan.name = "";
      initLoan.principal = null;
      initLoan.rate = null;
      initLoan.payment = null;
      initLoan.autoDeduct = false;
      initLoan.deductDay = 1;
      initLoan.deductAccountId = "";
    ...
    const openRateModal = (loan) => {
      ...
      rateData.rate = loan.interest_rate;
      showRateModal.value = true;
    };
    const submitRateAdjust = () => {
      if (!rateData.rate) return alert("請輸入利率");
      activeLoan.value.interest_rate = rateData.rate;
    ...
    const submitAddGoal = () => {
      if (!initGoal.name || !initGoal.target)
        return alert("請填寫目標名稱與金額");
      let tagClean = initGoal.tag
        ? initGoal.tag.replace("#", "").trim()
        : initGoal.name.replace(/\s+/g, "");

      if (initGoal.id) {
        // 編輯模式
        let g = data.savings_goals.find((x) => x && x.id === initGoal.id);
        if (g) {
          g.name = initGoal.name;
          g.tag = tagClean;
          g.target = initGoal.target;
          g.deadline = initGoal.deadline;
        }
      } else {
        // 新增模式
        data.savings_goals.push({
          ...
          name: initGoal.name,
          tag: tagClean,
          target: initGoal.target,
          deadline: initGoal.deadline,
          saved: 0,
        });
      }
      ...
      initGoal.name = "";
      initGoal.tag = "";
      initGoal.target = null;
      initGoal.deadline = "";
      autoBackup(true, true);
      alert("✅ 目標儲存成功！");
    };
    const openUpdateGoalModal = (goal) => {
      activeGoal.value = goal;
      updateGoalData.amount = null;
      updateGoalData.type = "add";
      showUpdateGoalModal.value = true;
    };
    const submitUpdateGoal = () => {
      if (!activeGoal.value || !updateGoalData.amount) return;
      if (updateGoalData.type === "add") {
        activeGoal.value.saved += updateGoalData.amount;
      } else {
        activeGoal.value.saved = updateGoalData.amount;
      }
      if (activeGoal.value.saved < 0) activeGoal.value.saved = 0;
      showUpdateGoalModal.value = false;
      autoBackup(true, true);
      alert("✅ 進度已更新！");
    };
    const deleteGoal = (id) => {
    ...
    const addRecurring = () => {
      if (!newRecurring.desc || !newRecurring.amount || !newRecurring.account)
        return alert("請填妥排程資訊");
      data.recurring.push({
        id: "rec_" + Date.now(),
        type: newRecurring.type,
        desc: newRecurring.desc,
        amount: newRecurring.amount,
        day: newRecurring.day,
        account: newRecurring.account,
      });
      newRecurring.desc = "";
      newRecurring.amount = null;
      newRecurring.day = 1;
      autoBackup(true, true);
      alert("✅ 排程建立成功！");
    };
    const deleteRecurring = (id) => {
      data.recurring = data.recurring.filter((r) => r && r.id !== id);
      autoBackup(true, true);
    };

    const addMainCategory = () => {
      let list = data.main_categories[settingCategoryMode.value] || [];
      if (newMainCat.value && !list.includes(newMainCat.value)) {
        data.main_categories[settingCategoryMode.value].push(newMainCat.value);
        newMainCat.value = "";
        autoBackup(true, true);
      }
    };
    const deleteMainCategory = (type, name) => {
      if (getSubAccounts(type, name, true).length > 0)
        return alert("請先清空子類別");
      data.main_categories[type] = (data.main_categories[type] || []).filter(
        (c) => c !== name,
      );
      autoBackup(true, true);
    };
    ...
    const addPreset = () => {
      let list = data.quick_tags || [];
      if (newPreset.value && !list.includes(newPreset.value)) {
        data.quick_tags.push(newPreset.value);
        newPreset.value = "";
        autoBackup(true, true);
      }
    };
    const removePreset = (idx) => {
      data.quick_tags.splice(idx, 1);
      autoBackup(true, true);
    };
    const toggleAccountVisibility = (id) => {
      let a = data.accounts.find((a) => a && a.id === id);
      if (a) {
    ...
    const runAutoTasks = () => {
        ...
        if (ld && ld.length >= 7) {
          ...
          while (true) {
            ...
            data.transactions.unshift({
              ...
              debits: [
                {
                  account_id: fa.expense_account_id,
                  amount: fa.monthly_depreciation,
                },
              ],
              credits: [
                {
                  account_id: fa.accumulated_dep_account_id,
                  amount: fa.monthly_depreciation,
                },
              ],
            });
      ...
      (data.installments || []).forEach((inst) => {
        ...
        while (inst.paid_periods < inst.periods && inst.next_month <= curM) {
          ...
          let amt =
            inst.paid_periods === 0 && inst.first_amount
              ? inst.first_amount
              : inst.amount_per_period;
          data.transactions.unshift({
            id: "tx_inst_" + Date.now() + Math.random(),
            date: `${inst.next_month}-${day}`,
            desc: `${inst.desc} (${inst.paid_periods + 1}/${inst.periods}期)`,
            scope: inst.scope,
            auto_generated: true,
            inst_id: inst.id,
            debits: [{ account_id: inst.debit_acc, amount: amt }],
            credits: [{ account_id: inst.credit_acc, amount: amt }],
          });
          inst.paid_periods++;
          let p = inst.next_month.split("-");
          let y = Number(p[0]);
      ...
      (data.recurring || []).forEach((rec) => {
        ...
        if (lastExec !== curM && today >= rec.day) {
          let txObj = {
            ...
            date: `${curM}-${String(rec.day).padStart(2, "0")}`,
            scope: "personal",
            desc: `[定期] ${rec.desc}`,
            debits: [],
            credits: [],
          ...
          if (rec.type === "expense") {
            let sub = data.accounts.find(
              (a) => a && a.type === "Expense" && a.name === rec.desc,
            );
            txObj.debits.push({
              account_id: sub ? sub.id : "5102",
              amount: rec.amount,
            });
            txObj.credits.push({ account_id: rec.account, amount: rec.amount });
          } else {
            let sub = data.accounts.find(
              (a) => a && a.type === "Income" && a.name === rec.desc,
            );
            txObj.debits.push({ account_id: rec.account, amount: rec.amount });
            txObj.credits.push({
              account_id: sub ? sub.id : "4201",
              amount: rec.amount,
            });
          }
          data.transactions.unshift(txObj);
    ...
    const executeFactoryReset = async () => {
      ...
      if (
        ...
        try {
          await fetch(
            `https://www.googleapis.com/upload/drive/v3/files/${settings.fileId}?uploadType=media`,
            {
              method: "PATCH",
              headers: {
      ...
      nextTick(() => {
        try {
          if (typeof renderExpenseChart === "function") {
            let p = activeBillingPeriod.value;
            let filteredTxs = data.transactions.filter(
              (tx) => tx && tx.date >= p.startDate && tx.date <= p.endDate,
            );
            expenseChartInstance.value = renderExpenseChart(
          ...
          (data.accounts || []).forEach((a) => {
            if (
              ...
              cTot += getBaseBalance(a.id, calculateBalance(a.id, scope));
          });
          (data.investments || []).forEach((inv) => {
            if (inv)
              sTot +=
          ...
          let fTot =
            calculateBalance("1201", scope) +
            calculateBalance("1201-DEP", scope);
          if (typeof renderAssetChart === "function") {
            assetChartInstance.value = renderAssetChart(
              assetChartInstance.value,
              "assetChart",
              cTot,
              sTot,
              fTot,
          ...
          for (let i = rangeMonths - 1; i >= 0; i--) {
            ...
            (data.accounts || []).forEach((a) => {
              ...
              (data.transactions || []).forEach((tx) => {
                ...
                if (scope !== "all" && tx.scope !== scope) return;
                if (tx.debits)
                  tx.debits.forEach((db) => {
                    if (db.account_id === a.id) bal += Number(db.amount) || 0;
                  });
                if (tx.credits)
                  tx.credits.forEach((cr) => {
                    if (cr.account_id === a.id) bal -= Number(cr.amount) || 0;
                  });
              });
              if (a.type === "Asset") aSum += getBaseBalance(a.id, bal);
              else if (a.type === "Liability")
                lSum += getBaseBalance(a.id, -bal);
            });
            aSum += scopeUnrealizedGain;
            histData.push(aSum - lSum);
          }
          if (typeof renderNetWorthChart === "function") {
            netWorthChartInstance.value = renderNetWorthChart(
              netWorthChartInstance.value,
              "netWorthChart",
              histLabels,
              histData,
            );
    ...
    return {
      ...
      executeRollover,
      showSharedSettlementModal,
      sharedData,
      closeSharedSettlementModal,
      activeSplitProjectId,
      groupSplitProjectForm,
      groupSplitRecordForm,
      groupSettleLedgerForm,
      activeSplitProject,
      activeSplitRecords,
      activeSplitBalances,
      activeSplitSettlements,
      openGroupSplitCenter,
      viewGroupSplitProject,
      backToSplitProjects,
      addSplitMemberField,
      removeSplitMemberField,
      editGroupSplitProject,
      saveGroupSplitProject,
      deleteGroupSplitProject,
      initGroupSplitRecordForm,
      editGroupSplitRecord,
      calculateGroupSplitRecord,
      saveGroupSplitRecord,
      ...
      filterByAccount,
      historyPreviousTab,
      clearHistoryFilterAndBack,
      showEditAccountModal,
      editingAccount,
      openEditAccountModal,
      saveEditAccount,
      executeDeleteAccountFromModal,
      newCurrencyCode,
      addCustomCurrency,
      ...
      getBaseBalance,
      accountsWithBalance,
      paymentAccountsWithBalance,
      assetAccountsWithBalance,
